package HelloPerlService::HelloPerlServiceServer;
...
sub call_method {
    ...
    my $ctx = HelloPerlService::HelloPerlServiceServerContext->new($self->{loggers}->{userlog},
...
{
    # Service HelloPerlService requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for HelloPerlService but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = HelloPerlService::HelloPerlServiceServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service HelloPerlService.");
    }
...
package HelloPerlService::HelloPerlServiceServerContext;

use strict;

=head1 NAME

HelloPerlService::HelloPerlServiceServerContext
...
package HelloPerlService::HelloPerlServiceServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = HelloPerlService::HelloPerlServiceServer->new(
